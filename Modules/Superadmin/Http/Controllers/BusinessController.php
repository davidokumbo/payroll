...
class BusinessController extends BaseController
{
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            // Adding user details to users table of client payroll system (not the users_superadmin table) added by Dominick on 25/09/2023
            $data = $request->only(['surname', 'first_name', 'last_name', 'username', 'email', 'password']);

            // Combine first_name and last_name into a single 'name' variable
            $data['name'] = $data['first_name'] . ' ' . $data['surname'] . ' ' . $data['last_name'];
            $data['password'] = Hash::make($data['password']);

            // Remove first_name and last_name from the data array
            unset($data['first_name']);
            unset($data['last_name']);
            unset($data['surname']);

            // Add the organization_id to the data
            $data['organization_id'] = $business->id; // Replace 234 with the desired organization_id value
    ...
    public function destroy($id)
    {
        ...
        try {
            ...
            $products_id = Product::where('organization_id', $id)->pluck('id')->toArray();
